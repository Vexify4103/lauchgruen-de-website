...
export function TournamentLiveStreamLinks({ streams, compact = false }: { streams: TournamentLiveStream[]; compact?: boolean }) {
		...
		<div className={`flex flex-wrap gap-2 ${compact ? "" : "mt-3"}`}>
			{streams.map((stream) => (
				<a
					...
					} ${compact ? "px-2.5 py-1 text-[10px] uppercase tracking-[0.12em]" : "px-3 py-2 text-xs"}`}
				>
					...
					{!compact && !stream.preview ? <span className="font-bold text-[#d8c4f6]/60">{stream.viewerCount.toLocaleString("de-DE")}</span> : null}
				</a>
			))}
		</div>
	);
