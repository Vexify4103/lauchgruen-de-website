	...
	return (
		<div className="pointer-events-none fixed inset-0 overflow-hidden bg-transparent text-white">
			<section className="nachtdienst-hud absolute bottom-[18px] left-[18px] h-[178px] w-[520px]">
				<div aria-hidden className="absolute inset-0 bg-[linear-gradient(105deg,rgba(15,8,27,.96)_0%,rgba(26,10,42,.94)_58%,rgba(46,20,17,.91)_100%)] [clip-path:polygon(0_0,96%_0,100%_18%,100%_100%,4%_100%,0_82%)]" />
				...
				<div className="nachtdienst-scene relative grid h-full grid-cols-[92px_1fr] gap-3 px-4 py-3">
					...
					<div className="min-w-0">
						...
						<div className="mt-3 grid grid-cols-[1fr_auto_auto] items-center gap-3 border-y border-white/8 bg-black/20 px-2 py-1.5">
							...
							<Metric label="LP Delta" value={`${lpDelta > 0 ? "+" : ""}${lpDelta}`} tone={lpDelta > 0 ? "text-[#ffd84d]" : lpDelta < 0 ? "text-[#ff7a2f]" : "text-white/70"} />
						</div>

						<div className="mt-2 flex items-center gap-2">
							<div className="mr-1 w-12 text-[7px] font-black uppercase leading-tight tracking-[0.18em] text-[#c9a7ff]/70">Recent<br />signal</div>
							{[0, 1, 2, 3, 4].map((index) => {
								const game = data.lastGames[index];
								return game ? (
									<div key={game.matchId} className={`relative size-[36px] overflow-hidden border bg-black/35 ${game.win ? "border-[#ffd84d]/80" : "border-[#ff7a2f]/85"}`}>
										{/* eslint-disable-next-line @next/next/no-img-element */}
							...
							<div className="ml-auto text-right font-mono text-[8px] font-bold uppercase tracking-[0.12em] text-white/32">ND-{String(games).padStart(2, "0")}</div>
						</div>
					</div>
				</div>
				)}
		...
		<div className="nachtdienst-scene relative grid h-full grid-cols-[112px_1fr] gap-4 px-4 py-3">
			...
			<div className="min-w-0 py-0.5">
				...
				<div className="mt-2 flex items-center gap-1.5">
					<div className="mr-1 w-12 text-[7px] font-black uppercase leading-tight tracking-[0.18em] text-[#c9a7ff]/70">Final<br />loadout</div>
