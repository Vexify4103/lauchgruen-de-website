...
function SmallChampionScene({
	...
	return (
		<div
			className={`obs-small-scene absolute inset-0 grid h-full grid-cols-[auto_1fr_auto] items-center gap-3 ${
				active ? "obs-small-scene-active opacity-100 blur-0" : "pointer-events-none translate-x-[16px] opacity-0 blur-md"
			}`}
		>
			{featured ? (
				<div
					className={`relative size-16 overflow-hidden rounded-xl border-2 ${
						featured.win ? "border-lime-300 shadow-[0_0_16px_rgba(132,204,22,0.5)]" : "border-rose-400 shadow-[0_0_16px_rgba(251,113,133,0.45)]"
					}`}
				>
					{/* eslint-disable-next-line @next/next/no-img-element */}
					...
					<div
						className={`absolute bottom-0 right-0 rounded-tl-md px-1.5 py-0.5 text-[9px] font-black ${
							featured.win ? "bg-lime-300 text-emerald-950" : "bg-rose-400 text-white"
						}`}
					>
						{featured.win ? "W" : "L"}
					</div>
				...
				{featured ? (
					<>
						...
						<div className="mt-1.5 flex gap-1">
							{featured.items.slice(0, 6).map((item, index) => (
