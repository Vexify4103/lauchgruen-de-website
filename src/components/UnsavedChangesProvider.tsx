...
import { createContext, type ReactNode, useCallback, useContext, useEffect, useEffectEvent, useId, useMemo, useRef, useState } from "react";
...
type PendingNavigation = { kind: "href"; href: string } | { kind: "back" } | { kind: "reload" };

export function UnsavedChangesProvider({ children }: { children: ReactNode }) {
	const router = useRouter();
	const pathname = usePathname();
	const [registrations, setRegistrations] = useState(() => new Map<string, SaveRegistration>());
	...
	const dirtyRegistrations = useMemo(() => [...registrations.values()].filter((registration) => registration.dirty), [registrations]);
	...
	useEffect(() => {
		if (hasUnsavedChanges && !sentinelActive.current) {
			window.history.pushState({ ...window.history.state, unsavedChangesSentinel: true }, "", window.location.href);
			sentinelActive.current = true;
			return;
		}

		if (!hasUnsavedChanges && sentinelActive.current && !pendingNavigation && !saving) {
	...
	useEffect(() => {
		function handleDocumentClick(event: MouseEvent) {
			...
			setPendingNavigation({
				kind: "href",
				href: destination.origin === window.location.origin ? `${destination.pathname}${destination.search}${destination.hash}` : destination.href,
			});
		}
	...
	async function saveAndContinue() {
		...
		for (const registration of [...registrations.values()].filter((entry) => entry.dirty)) {
			try {
				const saved = await registration.save();
				if (!saved) {
					setError(`„${registration.label}“ konnte nicht gespeichert werden. Bitte prüfe die Eingaben.`);
					setSaving(false);
					return;
				}
			} catch {
				setError(`„${registration.label}“ konnte nicht gespeichert werden. Bitte versuche es erneut.`);
				setSaving(false);
				return;
			}
		}
	...
	return (
		<UnsavedChangesContext.Provider value={contextValue}>
			{children}
			{pendingNavigation ? (
				<div role="dialog" aria-modal="true" aria-labelledby="unsaved-changes-title" className="fixed inset-0 z-[100] grid place-items-center px-5">
					<button
						type="button"
					...
					<div className="relative w-full max-w-lg overflow-hidden rounded-[2rem] border border-amber-200/24 bg-gradient-to-br from-[#15261c] via-[#0b1b13] to-[#04100b] p-6 shadow-2xl shadow-black/60">
						...
						<div className="relative">
							...
							<h2 id="unsaved-changes-title" className="mt-4 text-2xl font-black tracking-tight text-emerald-50">
								Ungespeicherte Änderungen
							</h2>
							<p className="mt-2 text-sm leading-6 text-emerald-100/66">Wenn du jetzt weitergehst, gehen deine letzten Änderungen verloren.</p>
...
export function useUnsavedChanges({ dirty, label, save }: SaveRegistration) {
