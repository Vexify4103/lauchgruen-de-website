		...
		<div ref={containerRef} className="relative">
			<button
				...
				className={`flex w-full items-center justify-between rounded-2xl border px-4 py-3 text-left text-sm outline-none transition ${
					open ? "border-lime-200/40 bg-black/30" : "border-white/10 bg-black/24 hover:border-lime-200/24"
				}`}
			>
				<span className={selected ? "font-bold text-emerald-50" : "text-emerald-100/34"}>{selected?.label ?? placeholder}</span>
				<svg
					aria-hidden
					viewBox="0 0 12 8"
						...
						return (
							<li
								...
								className={`flex cursor-pointer items-center justify-between rounded-xl px-3 py-2 text-sm transition ${
									isHighlighted ? "bg-lime-200/12 text-lime-50" : "text-emerald-100/80"
								} ${isSelected ? "font-black" : "font-bold"}`}
							>
								<span>{option.label}</span>
								{isSelected ? <span className="text-xs font-black uppercase tracking-[0.18em] text-lime-200/72">Selected</span> : null}
							</li>
						);
					})}
				</ul>
...
export function ThemedMultiSelect({
	...
	const selectedText = selected.length > 0 ? selected.map((option) => option.label).join(", ") : placeholder;
	...
	return (
		<div ref={containerRef} className="relative">
			...
			>
				<span className={selected.length > 0 ? "font-bold text-emerald-50" : "text-emerald-100/34"}>{selectedText}</span>
				<svg
					aria-hidden
					viewBox="0 0 12 8"
			...
			{name ? value.map((entry) => <input key={entry} type="hidden" name={name} value={entry} />) : null}
						...
						return (
							<button
								...
								className={`flex w-full items-center justify-between rounded-xl px-3 py-2 text-left text-sm font-bold transition ${
									isSelected ? "bg-lime-200/14 text-lime-50" : "text-emerald-100/80 hover:bg-lime-200/8 hover:text-lime-50"
								}`}
							>
								<span>{option.label}</span>
								<span
									className={`grid size-5 place-items-center rounded-md border text-[10px] ${
										isSelected ? "border-lime-200/50 bg-lime-200 text-emerald-950" : "border-white/12 text-emerald-100/35"
									}`}
								>
									{isSelected ? "OK" : ""}
								</span>
