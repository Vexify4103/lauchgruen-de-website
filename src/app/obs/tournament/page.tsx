...
export default async function ObsTournamentPage({ searchParams }: { searchParams: Promise<{ team?: string }> }) {
...
async function buildInitial(teamId: string, ctx: Awaited<ReturnType<typeof getTournamentContext>>): Promise<ObsTeamResponse | null> {
	const team = ctx.teams.find((t) => t.id === teamId);
	if (!team) return null;
	...
	const live = resolved.find((m) => (m.teamAName === team.name || m.teamBName === team.name) && m.status === "Live");
	const currentMatch: OverlayMatch | null = live ? toOverlay(live, team.name, poolFor(live.id)) : null;
...
function toOverlay(m: ReturnType<typeof resolvePlayoffMatches>[number], teamName: string, poolAssignment: WheelMatchAssignment | null): OverlayMatch {
...
function ObsHelp({ teams, message }: { teams: Awaited<ReturnType<typeof getTournamentContext>>["teams"]; message?: string }) {
	return (
		<div className="min-h-screen bg-emerald-950/80 p-6 font-mono text-sm text-emerald-100">
			<div className="rounded-2xl border border-white/10 bg-black/60 p-5">
				<div className="text-xs font-black uppercase tracking-[0.24em] text-lime-200/72">OBS Turnier-Overlay</div>
				{message ? <div className="mt-3 rounded-lg border border-red-300/30 bg-red-500/10 px-3 py-2 text-red-100">{message}</div> : null}
				<div className="mt-3 text-emerald-100/72">Browser-Source in OBS hinzufügen:</div>
				<ul className="mt-3 space-y-1 text-emerald-100/80">
					<li>
						URL: <code className="rounded bg-black/40 px-1.5 py-0.5">/obs/tournament?team=&lt;team-id&gt;</code>
					</li>
					<li>Empfohlene Größe: 480 × 200</li>
					<li>Custom CSS: leer lassen — Hintergrund ist bereits transparent.</li>
				</ul>
				<div className="mt-4 text-xs font-black uppercase tracking-[0.22em] text-lime-200/60">Verfügbare Team-IDs</div>
				<div className="mt-2 flex flex-wrap gap-2">
					{teams.map((t) => (
						<code key={t.id} className="rounded border border-white/10 bg-black/40 px-2 py-1 text-xs">
							{t.id}
						</code>
					))}
				</div>
