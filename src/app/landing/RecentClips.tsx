    ...
    (async () => {
      try {
        const response = await fetch(
          `/api/twitch/clips?login=${encodeURIComponent(login)}&count=${count}`,
          { cache: "no-store" },
        );
    ...
    return (
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {Array.from({ length: count }).map((_, index) => (
          <div
            key={index}
            className="animate-pulse rounded-[1.7rem] border border-white/10 bg-white/[0.045] p-4"
          >
  ...
  if (!clips || clips.length === 0) {
    return (
      <div className="rounded-[1.7rem] border border-white/10 bg-white/[0.045] p-6 text-sm leading-7 text-emerald-100/68">
        Noch keine Clips aus den letzten 30 Tagen. Sobald wieder live ist und der
        Chat den Highlight-Knopf drückt, tauchen sie hier auf.
      </div>
    );
  }
      ...
      {clips.map((clip) => (
        ...
        >
          ...
          <div className="flex flex-1 flex-col justify-between gap-3 p-4">
            <div className="line-clamp-2 text-sm font-black leading-snug text-emerald-50 group-hover:text-lime-100">
              {clip.title}
            </div>
