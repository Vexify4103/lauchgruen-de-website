...
function captainRoleId() {
	return process.env.DISCORD_CAPTAINS_ROLE_ID?.trim() || process.env.CAPTAIN_ROLE_ID?.trim() || "";
}
...
export async function POST(request: Request) {
	...
	if (!team || team.meta?.captain?.discordId !== currentDiscordId) {
		return NextResponse.json({ message: "Du bist nicht der aktuelle Captain dieses Teams." }, { status: 403 });
	}

	if (parsed.data.targetDiscordId === currentDiscordId) {
		return NextResponse.json({ message: "Du bist bereits Captain dieses Teams." }, { status: 400 });
	}

	const targetPlayer = (team.players ?? []).find((player) => player.discordId === parsed.data.targetDiscordId);
	if (!targetPlayer) {
		return NextResponse.json({ message: "Der ausgewählte Spieler gehört nicht zu deinem Team." }, { status: 400 });
	}
	const targetAlreadyCaptainsAnotherTeam = Object.entries(teams).some(
		([key, entry]) => key !== parsed.data.teamKey && entry.meta?.captain?.discordId === parsed.data.targetDiscordId
	);
	if (targetAlreadyCaptainsAnotherTeam) {
		return NextResponse.json({ message: "Der ausgewählte Spieler ist bereits Captain eines anderen Teams." }, { status: 409 });
	}

	const matchContext = await getMatchControlContext();
	const unfinishedMatches = matchContext.matches.filter(
		(match) => match.status !== "Finished" && match.status !== "Live" && (match.teamAName === team.name || match.teamBName === team.name)
	);
	const draftStates = await Promise.all(unfinishedMatches.map((match) => getDraftState(match.id)));
	const activeDraft = draftStates.find((draft) => draft.actions.length > 0 || Boolean(draft.pendingSelection) || Boolean(draft.readyBy.teamA) || Boolean(draft.readyBy.teamB));
	if (activeDraft) {
		return NextResponse.json(
			{
				message: `Captain kann nicht übertragen werden, solange der Draft ${activeDraft.matchId} läuft oder bereits ein Team ready ist.`,
			},
			{ status: 409 }
		);
	}
	...
	if (!roleId) {
		return NextResponse.json(
			{
				message: "Captain-Rolle ist nicht konfiguriert. DISCORD_CAPTAINS_ROLE_ID fehlt.",
			},
			{ status: 503 }
		);
	}
	...
	const nextCaptain: StoredCaptain = {
		discordId: parsed.data.targetDiscordId,
		discordUsername: targetApplication?.discordUsername ?? targetApplication?.discordHandle,
	...
	try {
		await writeAuditLog({
			...
			summary:
				`${session.user.discordHandle ?? currentDiscordId} hat den Captain von ` + `${team.name} an ${targetApplication?.displayName ?? targetPlayer.riotId} übertragen.`,
