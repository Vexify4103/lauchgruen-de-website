...
export async function POST() {
  ...
  const [botDoc, applications] = await Promise.all([
    db.collection<BotStateDoc>("bot_state").findOne({ _id: "default" }),
    listApplications(),
  ]);
  const applicationByDiscordId = new Map(
    applications.map((application) => [application.discordId, application]),
  );
  const players = Object.values(botDoc?.teams ?? {}).flatMap((team) =>
    (team.players ?? []).filter((player) => player.discordId),
  );
