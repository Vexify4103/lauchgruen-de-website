...
function publicGroup(
  group: Awaited<ReturnType<typeof getPreferenceGroupForDiscordId>>,
) {
...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { message: "Ungültige Wunschgruppen-Aktion." },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const group =
      parsed.data.action === "create"
        ? await createPreferenceGroup(discordId)
        : await joinPreferenceGroup(discordId, parsed.data.code);

    return NextResponse.json({
      group: publicGroup(group),
      message:
        parsed.data.action === "create"
          ? "Dein Wunschgruppen-Code ist bereit."
          : "Du bist der Wunschgruppe beigetreten.",
    });
  } catch (error) {
    ...
    const responses: Record<string, { message: string; status: number }> = {
      ...
      ALREADY_IN_GROUP: {
        message:
          "Du bist bereits in einer Wunschgruppe. Verlasse sie zuerst, um einem anderen Code beizutreten.",
        status: 409,
      },
      GROUP_FULL: {
