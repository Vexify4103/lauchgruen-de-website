...
export async function GET(request: Request) {
	...
	const teamGroupMatches: GroupMatch[] = ctx.groupMatches.filter((m) => m.group === team.group && (m.teamA === team.name || m.teamB === team.name));
	...
	function toOverlay(entry: (typeof allRelevant)[number]): OverlayMatch {
		...
		const bracket = entry.base.kind === "group" ? ("Group" as const) : entry.base.bracket;
		const round = entry.base.kind === "group" ? entry.base.match.round : entry.base.round;
		const id = entry.base.kind === "group" ? entry.base.match.id : entry.base.id;
		const time = entry.base.kind === "group" ? entry.base.match.time : entry.base.time;
		return {
			id,
			opponent,
	...
	const upcoming = allRelevant.find((e) => e.storedStatus !== "Finished" && e.storedStatus !== "Live" && (e.scoreA === undefined || e.scoreB === undefined));
	// Latest finished match (last in tournament-data declaration order with scores).
	const finishedEntries = allRelevant.filter((e) => e.scoreA !== undefined && e.scoreB !== undefined);
