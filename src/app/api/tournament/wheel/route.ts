...
import { getTournamentWheelState, resetTournamentWheel, spinTournamentWheelForMatch } from "@/lib/tournament-wheel";
...
export async function POST(request: Request) {
	...
	try {
		...
		await writeAuditLog({
			...
			summary: parsed.data.action === "reset" ? "A-Z wheel reset." : `Pools drawn for ${parsed.data.matchId}.`,
		...
		return NextResponse.json({ message: error instanceof Error ? error.message : "Wheel konnte nicht gespeichert werden." }, { status: 400 });
	}
}

async function spinAfterMatchGuard(input: { matchId: string; teamAName: string; teamBName: string; spunBy?: string }) {
