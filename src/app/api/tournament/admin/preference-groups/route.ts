...
import {
  ...
  adminCreatePreferenceGroup,
  adminMovePreferenceGroupMember,
...
async function writeAdminAudit(entry: Parameters<typeof writeAuditLog>[0]) {
  try {
    ...
    console.error(
      "[preference-groups] Audit-Log konnte nicht geschrieben werden.",
      error,
    );
  }
}
...
export async function POST(request: Request) {
  ...
  if (
    !actorDiscordId
    || !TOURNAMENT_OWNER_DISCORD_IDS.has(actorDiscordId)
  ) {
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { message: "Ungültige Gruppendaten." },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const group = await adminMovePreferenceGroupMember(
      parsed.data.discordId,
      parsed.data.targetCode,
    );
    await writeAdminAudit({
      action: group
        ? "preference_group.member_moved_by_admin"
        : "preference_group.member_removed_by_admin",
      targetType: "preference_group",
      targetId: group?.code ?? parsed.data.discordId,
      summary: group
        ? `Person wurde der Wunschgruppe ${group.code} zugewiesen.`
        : "Person wurde aus ihrer Wunschgruppe entfernt.",
      actorDiscordId,
      actorLabel: session.user.discordHandle ?? actorDiscordId,
      metadata: {
    ...
    const responses: Record<string, { message: string; status: number }> = {
      INVALID_GROUP_SIZE: {
        message: "Eine Wunschgruppe braucht eine bis fünf Personen.",
        status: 400,
      },
      ...
      GROUP_FULL: {
        message: "Die Zielgruppe ist bereits voll.",
        status: 409,
      },
