...
const applicationPatchSchema = z.object({
	...
	manualRankOverride: z.string().trim().max(40).nullable().optional(),
});
...
export async function POST(request: Request) {
	...
	if (!discordId || !discordHandle) {
		return NextResponse.json({ message: "Bitte zuerst mit Discord anmelden, bevor du dich bewirbst." }, { status: 401 });
	}
	...
	if (blacklistMatch) {
		return NextResponse.json(
			{
				message: "Dieser Discord- oder Riot-Account ist für Lauchgruen-Turniere gesperrt. Bitte melde dich beim Orga-Team, falls du glaubst, dass das ein Fehler ist.",
			},
			{ status: 403 }
		);
	}
	...
	return NextResponse.json({
		message: existing ? "Bewerbung aktualisiert. Danke, dass du die Infos aktuell hältst." : "Bewerbung gespeichert. Willkommen in der Warteliste.",
	});
}
...
export async function GET(request: Request) {
	...
	const versionFor = url.searchParams.get("versionFor");
	...
	if (versionFor) {
		const version = await import("@/lib/admin-version").then((m) => m.getAdminVersion(`application:${versionFor}`));
...
export async function DELETE(request: Request) {
	...
	if (!targetDiscordId || !applicationId || !Number.isInteger(expectedVersion) || expectedVersion < 0) {
		return NextResponse.json({ message: "Bewerbungs-ID, Discord-ID und Version sind erforderlich." }, { status: 400 });
	}
