...
function LiveMatchCard({
	...
	return (
		<article className={`rounded-[1.8rem] border p-5 shadow-xl shadow-black/20 ${active ? "border-red-300/28 bg-red-500/[0.08]" : "border-white/10 bg-white/[0.045]"}`}>
			...
			<h3 className="mt-4 text-2xl font-black text-emerald-50">
			...
			<div className="mt-5 flex flex-wrap gap-2">
				<Link
					...
					className="rounded-xl border border-white/12 bg-black/18 px-3 py-2 text-xs font-black uppercase tracking-[0.14em] text-emerald-100 hover:text-lime-100"
				>
					Match-Details
				</Link>
				{match.poolAssignment ? (
					<Link
						...
						className="rounded-xl border border-sky-200/20 bg-sky-300/10 px-3 py-2 text-xs font-black uppercase tracking-[0.14em] text-sky-100"
					>
						Draft ansehen
					</Link>
				) : null}
