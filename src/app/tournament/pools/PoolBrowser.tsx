...
export function PoolBrowser({ pools }: { pools: ChampionPool[] }) {
  ...
  const visiblePools = useMemo(
    () => selectedPool === "all"
      ? pools
      : pools.filter((pool) => pool.pool === selectedPool),
    [pools, selectedPool],
  );
    ...
    <div className="mt-8 grid gap-5">
      <div className="sticky top-24 z-20 rounded-[1.5rem] border border-white/10 bg-[#07110c]/88 p-3 shadow-2xl shadow-black/30 backdrop-blur-xl">
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className={filterClass(selectedPool === "all")}
          >
            Alle
          </button>
          {pools.map((pool) => (
      ...
      <div className="grid gap-4">
        {visiblePools.map((pool, index) => (
          <article
            key={pool.pool}
            className="overflow-hidden rounded-[1.4rem] border border-white/10 bg-white/[0.04] shadow-xl shadow-black/18"
          >
              ...
              <div className="grid grid-cols-4 gap-2 p-3 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 xl:grid-cols-12">
                {pool.champions.map((champion) => (
                  ...
                  >
                    ...
                    <div className="mt-1 truncate text-center text-[11px] font-black text-emerald-50">
                      {champion.name}
                    </div>
                  </div>
                ))}
...
function filterClass(active: boolean) {
  return `rounded-xl border px-3 py-2 text-xs font-black uppercase tracking-[0.14em] transition ${
    active
      ...
      : "border-white/10 bg-black/22 text-emerald-100/58 hover:border-lime-200/26 hover:text-lime-100"
  }`;
}
