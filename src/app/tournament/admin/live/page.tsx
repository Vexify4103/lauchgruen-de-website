        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div className="max-w-3xl">
            ...
            <p className="mt-4 text-sm leading-7 text-emerald-100/68">
              Eine reduzierte Turniertag-Ansicht: aktive Matches, Draft-Status,
              fehlende Scores, nächste Matches und OBS-Hinweise.
            </p>
          </div>
          <Link
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  {next.map((match) => (
                    <MatchCard
                      ...
                      href={`/tournament/admin/matches/${match.id}`}
                      badges={[
                        match.poolAssignment ? "Pools gezogen" : "Pools offen",
          ...
          <aside className="grid content-start gap-6">
            ...
            <Panel title="OBS">
              <p className="text-sm leading-6 text-emerald-100/62">
                Team-Overlays lesen `/api/tournament/obs?team=TEAM_ID` und zeigen
                auch den aktuellen Pool, sobald er gezogen wurde.
              </p>
              <Link
                href="/obs/tournament"
...
function MatchCard({
  ...
  return (
    <Link
      href={href}
      className="block rounded-2xl border border-white/10 bg-black/18 p-4 transition hover:-translate-y-0.5 hover:border-lime-200/28"
    >
...
function Empty({ text }: { text: string }) {
  return (
    <div className="rounded-2xl border border-white/8 bg-black/18 p-4 text-sm font-bold text-emerald-100/48">
      {text}
    </div>
  );
}
