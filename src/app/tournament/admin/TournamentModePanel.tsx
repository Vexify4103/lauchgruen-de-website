					...
					return (
						<button
							...
							className={`rounded-2xl border p-4 text-left transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-55 ${
								active ? "border-lime-200/24 bg-lime-200/10" : "border-white/10 bg-black/18"
							}`}
						>
							<div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-100/48">{item.label}</div>
							<div className={`mt-2 text-xl font-black ${active ? "text-lime-100" : "text-emerald-100/52"}`}>{active ? item.on : item.off}</div>
						</button>
					);
				})}
			</div>
