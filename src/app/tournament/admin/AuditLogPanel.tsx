		...
		<section className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-5 shadow-xl shadow-black/24">
			<div className="flex flex-wrap items-end justify-between gap-3">
				...
				<div className="flex flex-wrap items-center gap-2">
					<div className="rounded-2xl border border-white/10 bg-black/18 px-4 py-2 text-sm font-black text-emerald-100/48">max. 5 sichtbar</div>
					<button
						type="button"
			...
			{entries.length === 0 ? (
				<p className="mt-4 rounded-2xl border border-white/8 bg-black/16 p-4 text-sm text-emerald-100/48">Noch keine Aktionen gespeichert.</p>
			) : (
				<div className="mt-4 max-h-80 overflow-y-auto pr-2">
					<div className="grid gap-2">
						{entries.map((entry) => (
							<div key={entry.id} className="grid gap-2 rounded-2xl border border-white/8 bg-black/18 p-3 md:grid-cols-[9rem_1fr_auto_auto] md:items-center">
								<div className="text-[10px] font-black uppercase tracking-[0.18em] text-lime-200/54">{entry.action}</div>
								...
								<div className="text-xs font-bold text-emerald-100/42">{new Date(entry.createdAt).toLocaleTimeString("de-DE")}</div>
								<button
									type="button"
									disabled={isPending}
