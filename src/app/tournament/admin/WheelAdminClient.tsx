...
export function WheelAdminClient({
  ...
  initialState: TournamentWheelState;
  matches: AdminMatch[];
}) {
  ...
  const hasActiveDrawForSelected = Boolean(
    selectedMatch && state.currentAssignment?.matchId === selectedMatch.id,
  );
  const selectedMatchCompleted = Boolean(
    selectedMatch
      && (selectedMatch.status === "Finished"
        || state.completedMatchIds.includes(selectedMatch.id)),
  );
  ...
  function spin() {
    ...
    startTransition(async () => {
      ...
      window.setTimeout(() => {
        ...
        setState(json);
        setMessage(
          `${assignment.teamAName}: ${compactPoolLabel(assignment.teamAPool)} · ${assignment.teamBName}: ${compactPoolLabel(assignment.teamBPool)}`,
        );
        setSpinning(false);
      }, SPIN_DURATION_MS);
    });
    ...
    <section className="rounded-[2rem] border border-lime-200/12 bg-white/[0.045] p-5 shadow-xl shadow-black/24">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="text-xs font-black uppercase tracking-[0.28em] text-lime-200/64">
            A-Z Wheel of Fortune
          </div>
          <h2 className="mt-2 text-2xl font-black text-emerald-50">
            Pools pro Match ziehen
          </h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-emerald-100/64">
            Wähle ein Match aus. Das Wheel zieht getrennt für Team A und Team B
            je einen Pool, den das jeweilige Team noch nicht gespielt hat.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
        <label className="grid gap-2">
          <span className="text-xs font-black uppercase tracking-[0.24em] text-lime-200/58">
            Match
          </span>
          <select
        ...
        <button
          ...
          disabled={
            ...
            || hasActiveDrawForSelected
            || teamARemaining.length === 0
            || teamBRemaining.length === 0
          }
      ...
      {state.history.length > 0 ? (
        <div className="mt-5 rounded-2xl border border-white/8 bg-black/18 p-4">
          <div className="text-[10px] font-black uppercase tracking-[0.24em] text-lime-200/56">
            Verlauf
          </div>
  ...
  return (
    <div className="rounded-[1.7rem] border border-white/10 bg-black/18 p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-xs font-black text-emerald-100/56">
          {remaining} übrig
        </div>
      </div>
      ...
      <div className="mt-4 grid place-items-center">
        <div className="relative grid size-56 place-items-center rounded-full border border-lime-200/20 bg-black/28 shadow-xl shadow-black/30">
          ...
          <div className="relative z-10 grid size-24 place-items-center rounded-full border border-lime-200/24 bg-emerald-950/92 text-center shadow-xl shadow-black/40">
            <span className="px-2 text-2xl font-black text-lime-100">
              {spinning ? "..." : pool ? compactPoolLabel(pool) : "?"}
            </span>
          </div>
        </div>
      </div>
