...
export function PreferenceGroupManager({
  ...
  const applicantById = new Map(
    applicants.map((applicant) => [applicant.discordId, applicant]),
  );
  ...
  const assignmentDirty = Boolean(
    selectedApplicant
    && targetCode !== (selectedApplicantEntry?.groupCode ?? ""),
  );
  ...
  async function createGroup(): Promise<boolean> {
    const saved = await request(
      { action: "create", discordIds: newMembers },
      "Wunschgruppe erstellt.",
    );
    if (saved) {
      setNewMembers([]);
    }
  ...
  return (
    <section className="mt-8 rounded-[2rem] border border-cyan-200/16 bg-cyan-300/[0.045] p-5 shadow-xl shadow-black/20">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="text-xs font-black uppercase tracking-[0.28em] text-cyan-100/68">
            Wunschgruppen verwalten
          </div>
          <h2 className="mt-2 text-2xl font-black text-emerald-50">
            Gemeinsam spielen – fair eingeteilt
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-emerald-100/58">
            Hier könnt ihr Bewerber zu Wunschgruppen zusammenfassen oder ihre
            Gruppenzugehörigkeit anpassen. Der Auto-Balancer berücksichtigt
            diese Wünsche nach Möglichkeit, eine gemeinsame Einteilung ist
      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-2">
        <div className="rounded-2xl border border-white/9 bg-black/18 p-4">
          <div className="text-sm font-black text-emerald-50">
            Neue Wunschgruppe erstellen
          </div>
          <p className="mt-1 text-xs leading-5 text-emerald-100/48">
            Wähle eine bis fünf Bewerber aus, die noch keiner Gruppe angehören.
          </p>
          <div className="mt-4">
            <ThemedMultiSelect
              value={newMembers}
          ...
          >
            {isPending
              ? "Wird gespeichert..."
              : `Gruppe mit ${newMembers.length} erstellen`}
          </button>
        </div>

        <div className="rounded-2xl border border-white/9 bg-black/18 p-4">
          <div className="text-sm font-black text-emerald-50">
            Person zuweisen oder verschieben
          </div>
          <p className="mt-1 text-xs leading-5 text-emerald-100/48">
            „Keine Wunschgruppe“ entfernt die Person aus ihrer aktuellen Gruppe.
          </p>
          <div className="mt-4 grid gap-3">
            <ThemedSelect
              ...
              onChange={(value) => {
                ...
                setTargetCode(
                  applicants.find((entry) => entry.discordId === value)
                    ?.groupCode ?? "",
                );
              }}
              placeholder="Bewerber auswählen"
      ...
      {groups.length > 0 ? (
        <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {groups.map((group) => (
            <article
              key={group.code}
              className="rounded-2xl border border-cyan-200/12 bg-black/18 p-4"
            >
              <div className="flex items-center justify-between gap-3">
                <span className="font-mono text-sm font-black tracking-[0.12em] text-cyan-50">
                  {group.code}
                </span>
                <span className="text-[10px] font-black text-cyan-100/48">
                  {group.memberDiscordIds.length}/5
                </span>
              </div>
      ...
      {message ? (
        <div className="mt-4 rounded-xl border border-cyan-200/16 bg-black/20 px-4 py-3 text-xs font-bold text-cyan-50/80">
          {message}
        </div>
      ) : null}
    </section>
