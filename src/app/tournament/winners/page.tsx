...
export const metadata: Metadata = {
  ...
  description:
    "Vergangene Lauchgruen Turniersieger, Finalisten und besondere Turniermomente.",
};
    ...
    <div className="px-5 py-10 sm:py-14">
      <section className="mx-auto w-full max-w-7xl">
        <div className="overflow-hidden rounded-[2.4rem] border border-amber-200/16 bg-gradient-to-br from-amber-200/12 via-lime-300/8 to-cyan-300/8 p-6 shadow-2xl shadow-black/30 sm:p-8 lg:p-10">
          ...
          <h1 className="mt-7 max-w-4xl text-5xl font-black leading-[0.94] tracking-tight text-emerald-50 sm:text-6xl">
            Die Lauchgruen Turnier-Champions.
          </h1>
        ...
        {pastTournamentWinners.length === 0 ? (
          <div className="mt-6 rounded-[2rem] border border-white/10 bg-white/[0.045] p-6 shadow-xl shadow-black/20 sm:p-8">
            ...
            <h2 className="mt-3 text-3xl font-black text-emerald-50">
              Der erste Eintrag wartet auf den Gewinner von {tournament.season}.
            </h2>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-3xl border border-white/10 bg-black/18 p-5">
      <div className="text-xs font-black uppercase tracking-[0.26em] text-amber-100/58">
        {label}
      </div>
...
function WinnerCard({
  ...
  entry: (typeof pastTournamentWinners)[number];
  featured?: boolean;
}) {
  return (
    <article
      className={`overflow-hidden rounded-[2rem] border bg-white/[0.045] shadow-xl shadow-black/20 ${
        featured ? "border-amber-200/24" : "border-white/10"
      }`}
    >
      <div className="border-b border-white/10 bg-gradient-to-r from-amber-200/12 via-lime-300/8 to-transparent p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <span className="text-xs font-black uppercase tracking-[0.18em] text-emerald-100/46">
            {entry.date}
          </span>
        </div>
      ...
      <div className="p-5">
        ...
        <div className="mt-5">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {entry.roster.map((player) => (
              <span
                key={player}
                className="rounded-2xl border border-white/10 bg-black/18 px-3 py-2 text-sm font-bold text-emerald-100/78"
              >
                {player}
              </span>
            ))}
        ...
        {entry.note ? (
          <p className="mt-5 rounded-2xl border border-lime-200/12 bg-lime-200/[0.055] p-4 text-sm leading-7 text-emerald-100/70">
            {entry.note}
          </p>
        ) : null}
      </div>
...
function Info({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-white/8 bg-black/16 p-4">
      <div className="text-[10px] font-black uppercase tracking-[0.22em] text-emerald-100/42">
        {label}
      </div>
