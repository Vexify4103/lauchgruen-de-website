...
export function TournamentAccountControl({
  account,
  cleanUrls,
  compact = false,
}: {
  account: TournamentAccount | null;
  cleanUrls: boolean;
  compact?: boolean;
}) {
    ...
    >
      {account.discordAvatar ? (
        ...
        <span className="text-sm font-black text-lime-100">
          {initials(account.discordHandle)}
        </span>
      )}
      <span
        className={`absolute -bottom-0.5 -right-0.5 size-3.5 rounded-full border-2 border-[#07110c] ${
          account.discordInGuild === false ? "bg-amber-300" : "bg-lime-300"
        }`}
      />
    </Link>
  );
