      ...
      <section className="mx-auto w-full max-w-7xl">
            ...
            <div className="flex flex-col justify-between p-7 sm:p-10 lg:p-12">
              <div>
                ...
                <h1 className="mt-8 max-w-4xl text-5xl font-black leading-[0.92] tracking-[-0.055em] text-emerald-50 sm:text-6xl lg:text-[5.25rem]">
                  Ein Buchstabe.
                  <span className="block bg-gradient-to-r from-lime-200 via-emerald-200 to-cyan-200 bg-clip-text text-transparent">
                    Ein Champion-Pool.
                  </span>
                  Sehr viel Chaos.
                </h1>
                <p className="mt-6 max-w-2xl text-base leading-8 text-emerald-100/68 sm:text-lg">
                  Zwei Abende League of Legends mit fair ausgelosten Teams,
                  einem eigenen Champion-Pool pro Match und einem Endbracket,
                  in dem jede Gruppenplatzierung zählt.
                </p>
              ...
              <div className="relative flex h-full flex-col justify-between gap-7">
                <div>
                  <div className="flex items-center justify-between gap-4">
                    <span className="text-[10px] font-black uppercase tracking-[0.3em] text-lime-200/54">
                      Kunterbuntes A-Z Turnier
                    </span>
                    ...
                    <div className="flex items-end justify-between gap-4">
                      <div>
                        ...
                        <p className="mt-3 max-w-xs text-sm leading-6 text-emerald-100/58">
                          Zwei Teams. Zwei gezogene Pools. Keine Wiederholung bis
                          zum Reset am zweiten Spieltag.
                        </p>
                      </div>
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1.08fr)_minmax(24rem,0.92fr)]">
          <section className="h-full rounded-[2rem] border border-lime-200/12 bg-white/[0.04] p-6 shadow-xl shadow-black/20 sm:p-8">
            ...
            <div className="mt-6 grid gap-3 md:grid-cols-3">
              {formatSteps.map((step) => (
                ...
                >
                  <div className="text-3xl font-black text-lime-200/28">
                    {step.number}
                  </div>
                  <h2 className="mt-5 text-xl font-black text-emerald-50">
                    {step.label}
                  </h2>
                  <p className="mt-3 text-sm leading-6 text-emerald-100/62">
                    {step.text}
                  </p>
                </article>
              ))}
          ...
          <aside className="h-full rounded-[2rem] border border-cyan-200/12 bg-cyan-300/[0.045] p-6 shadow-xl shadow-black/20 sm:p-8">
            <div className="text-xs font-black uppercase tracking-[0.28em] text-cyan-100/64">
              Bewerbung
            </div>
            <div className="mt-5 grid gap-x-5 gap-y-4 sm:grid-cols-2">
              {applicationSteps.map((step, index) => (
                <div key={step} className="flex items-start gap-3">
                  ...
                  <p className="pt-0.5 text-sm leading-6 text-emerald-100/66">
                    {step}
                  </p>
                </div>
              ))}
            ...
            <div className="mt-6 rounded-2xl border border-amber-200/16 bg-amber-200/[0.06] px-4 py-3 text-xs font-bold leading-5 text-amber-50/76">
              Verbindlich für beide Tage. Discord-Mitgliedschaft und ein
              verifizierter Riot-Account sind erforderlich.
            </div>
          </aside>
        </div>
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[0.82fr_1.18fr]">
          <section className="flex h-full flex-col rounded-[2rem] border border-white/10 bg-white/[0.04] p-6 shadow-xl shadow-black/20">
            ...
            <h2 className="mt-3 text-3xl font-black tracking-tight text-emerald-50">
              Das Rad bestimmt dein Champion-Roster.
            </h2>
            <p className="mt-3 text-sm leading-7 text-emerald-100/64">
              Jeder Pool gilt nur für ein Team in einem Match. Gespielte Pools
              verschwinden bis zum Reset am zweiten Spieltag aus diesem Team-Rad.
            </p>
            <Link
          ...
          <section className="h-full rounded-[2rem] border border-lime-200/10 bg-[#0a1810]/82 p-5 shadow-xl shadow-black/20">
            <div className="grid h-full grid-cols-3 gap-2 sm:grid-cols-4 lg:grid-cols-6">
              {azLetterPools.map((pool, index) => (
                ...
                >
                  <div className="text-[9px] font-black uppercase tracking-[0.18em] text-emerald-100/32">
                    Pool {String(index + 1).padStart(2, "0")}
                  </div>
                  <div className="mt-2 text-lg font-black text-lime-100">
                    {pool}
                  </div>
                </div>
              ))}
        ...
        <section className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <DashboardCard
            ...
            title="Rosters"
            text="Spieler, Rollen und OP.GG Multisearch."
          />
          <DashboardCard
...
function HeroFact({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <div className="text-[9px] font-black uppercase tracking-[0.2em] text-emerald-100/36">
        {label}
      </div>
...
function DateCard({
  ...
  return (
    <div className="rounded-[1.4rem] border border-white/12 bg-[#07110c]/82 p-4 shadow-2xl shadow-black/30 backdrop-blur-xl">
      <div className="text-[9px] font-black uppercase tracking-[0.22em] text-lime-200/58">
        {day}
      </div>
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="text-[10px] font-black uppercase tracking-[0.24em] text-lime-200/58">
          {label}
        </div>
        <div className="text-3xl font-black text-lime-200/24">{value}</div>
      </div>
      <h2 className="mt-5 text-xl font-black text-emerald-50 group-hover:text-lime-100">
        {title}
      </h2>
