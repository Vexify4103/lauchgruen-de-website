...
export function TournamentUrlProvider({ children, cleanUrls }: { children: ReactNode; cleanUrls: boolean }) {
	return <TournamentUrlContext.Provider value={cleanUrls}>{children}</TournamentUrlContext.Provider>;
}
...
type TournamentLinkProps = LinkProps & Omit<AnchorHTMLAttributes<HTMLAnchorElement>, keyof LinkProps | "href">;

export function TournamentLink({ href, ...props }: TournamentLinkProps) {
	const cleanUrls = useContext(TournamentUrlContext);
	const normalizedHref = typeof href === "string" ? cleanTournamentHref(href, cleanUrls) : href;
