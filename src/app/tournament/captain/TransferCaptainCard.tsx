...
export function TransferCaptainCard({
  teamKey,
  candidates,
}: {
  teamKey: string;
    ...
    return (
      <section className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-5 shadow-xl shadow-black/20">
        ...
        <p className="mt-2 text-sm leading-6 text-emerald-100/58">
          Aktuell gibt es kein weiteres Teammitglied mit verknüpftem
          Discord-Account.
        </p>
      </section>
    );
  ...
  return (
    <form
      onSubmit={submit}
      className="rounded-[2rem] border border-amber-200/16 bg-amber-300/[0.055] p-5 shadow-xl shadow-black/20"
    >
      ...
      <h2 className="mt-2 text-2xl font-black text-emerald-50">
        Verantwortung an ein Teammitglied abgeben
      </h2>
      <p className="mt-2 text-sm leading-6 text-emerald-100/58">
        Die Captain-Rolle, das Captain-Portal und alle Draft-Rechte wechseln
        sofort. Während eines bereits gestarteten Drafts ist die Übergabe
        gesperrt.
      </p>
      ...
      <div className="mt-4 grid gap-3">
        ...
        {candidate ? (
          <label className="grid gap-2">
            <span className="text-xs font-black uppercase tracking-[0.16em] text-emerald-100/52">
              Zur Bestätigung „{candidate.name}“ eingeben
            </span>
            <input
              value={confirmation}
      ...
      <button
        ...
        disabled={
          saving
          || !candidate
          || confirmation.trim() !== candidate.name
        }
