...
export function CaptainMatchActions({ matchId, teamName }: { matchId: string; teamName: string }) {
	...
	const [declaredWinner, setDeclaredWinner] = useState("yes");
	...
	function checkIn() {
		startTransition(async () => {
			const response = await fetch("/api/tournament/next", {
				...
				body: JSON.stringify({ action: "check-in", matchId, rosterConfirmed: true, rulesConfirmed: true }),
			});
			const json = (await response.json().catch(() => null)) as { message?: string } | null;
			if (!response.ok) return setMessage(json?.message ?? "Check-in konnte nicht gespeichert werden.");
			setCheckedIn(true);
			setMessage("Check-in gespeichert. Team und Regeln sind bestätigt.");
		});
	}
		...
		startTransition(async () => {
			const response = await fetch("/api/tournament/next", {
				...
				body: JSON.stringify({
					action: "report",
					matchId,
					declaredWinner: formData.get("winner") === "yes",
					gameDuration: String(formData.get("duration") ?? ""),
					screenshotUrl: String(formData.get("screenshot") ?? ""),
			...
			setReportOpen(false);
			setMessage("Match-Report gesendet. Die Orga kann ihn jetzt prüfen.");
		});
	}

	return (
		<div className="mt-4 rounded-2xl border border-cyan-200/14 bg-cyan-300/[0.055] p-4">
			<div className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-100/68">Captain-Check-in · {teamName}</div>
			<div className="mt-3 flex flex-wrap gap-2">
				<button
					type="button"
					onClick={checkIn}
					disabled={pending || checkedIn}
					className="rounded-xl border border-lime-200/24 bg-lime-200/10 px-4 py-2.5 text-xs font-black uppercase tracking-[0.14em] text-lime-100 disabled:opacity-50"
				>
					{checkedIn ? "Eingecheckt" : "Team bereit bestätigen"}
				</button>
				<button
					type="button"
					onClick={() => setReportOpen((open) => !open)}
					disabled={pending}
					className="rounded-xl border border-white/12 bg-black/18 px-4 py-2.5 text-xs font-black uppercase tracking-[0.14em] text-emerald-100"
				>
					Match-Report
				</button>
			</div>
			{reportOpen ? (
				<form action={submitReport} className="mt-4 grid gap-3">
					<label className="text-xs font-bold text-emerald-100/70">
						Hat {teamName} gewonnen?
						<div className="mt-1">
							<ThemedSelect
								name="winner"
								value={declaredWinner}
								onChange={setDeclaredWinner}
								ariaLabel={`Hat ${teamName} gewonnen?`}
								options={[
									{ value: "yes", label: "Ja" },
									{ value: "no", label: "Nein" },
								]}
