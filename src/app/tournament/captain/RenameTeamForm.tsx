...
export function RenameTeamForm({ teamKey, initialName }: { teamKey: string; initialName: string }) {
	...
	async function saveName(): Promise<boolean> {
		...
		setMessage({
			...
			text: warnings.length > 0 ? `Teamname geändert. ${warnings.join(" ")}` : "Teamname und Discord-Ressourcen wurden geändert.",
		});
	...
	return (
		<form onSubmit={submit} className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-5 shadow-xl shadow-black/20">
			<div className="text-xs font-black uppercase tracking-[0.28em] text-lime-200/64">Teamname</div>
