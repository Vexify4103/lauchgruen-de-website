...
export function DraftSpectatorClient({
  ...
  const candidatePool = currentTurn
    ? championsForTurn(currentTurn, blueChampions, redChampions)
    : [];
            ...
            {complete ? (
              ...
              <div className="grid grid-cols-4 gap-2 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-7 2xl:grid-cols-10">
                {candidatePool.map((champion) => {
                  ...
                  const pending = pendingSelection?.champion === champion.name
                    && pendingSelection.side === currentTurn?.side
                    && pendingSelection.kind === currentTurn?.kind;
                  return (
                    <div
                      key={champion.id}
  ...
  if (!ready) {
    return (
      <div className="mx-auto w-fit border-t-4 border-lime-300/75 px-8 pt-2 text-center">
        <p className="text-xs font-black uppercase tracking-[0.22em] text-emerald-50">
          Waiting for drafter
        </p>
        <p className="mt-1 text-[11px] font-bold text-emerald-100/48">
          Beide Captains müssen ready sein.
        </p>
      </div>
    );
  }
      ...
      <div className="mt-2 h-1 overflow-hidden rounded-full bg-white/10">
        <div
          className={`h-full rounded-full transition-all ${
            timer.remainingMs <= 0 ? "bg-amber-200" : isRed ? "bg-red-400" : "bg-sky-400"
          }`}
...
function DraftLane({
  ...
  const activePickIndex = currentTurn?.side === side && currentTurn.kind === "pick"
    ? picks.length
    : -1;
  const activeBanIndex = currentTurn?.side === side && currentTurn.kind === "ban"
    ? bans.length
    : -1;
  const pendingChampion = pendingSelection?.side === side
    ? byName.get(pendingSelection.champion)
    : undefined;

  return (
    <article className={`sticky top-4 border-0 bg-transparent p-0 shadow-none xl:row-span-3 ${
      isBlue ? "text-sky-50" : "text-red-50"
    }`}>
