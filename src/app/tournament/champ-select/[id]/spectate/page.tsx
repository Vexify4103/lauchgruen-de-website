...
export default async function DraftSpectatorPage({ params }: { params: Promise<{ id: string }> }) {
		...
		return (
			<div className="px-5 py-10 sm:py-14">
				<section className="mx-auto max-w-3xl rounded-[2rem] border border-amber-200/18 bg-amber-200/8 p-6 text-amber-50">
					<div className="text-xs font-black uppercase tracking-[0.28em] text-amber-100/70">Spectator Draft pausiert</div>
					<h1 className="mt-3 text-3xl font-black">Draft-Ansicht ist aktuell deaktiviert.</h1>
					<p className="mt-3 text-sm leading-7 text-amber-50/72">
						Das Orga-Team hat Champ Select gerade pausiert. Die Ansicht ist wieder erreichbar, sobald der Draft-Schalter aktiv ist.
					</p>
				</section>
			</div>
		);
