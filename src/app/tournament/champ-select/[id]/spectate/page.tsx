...
export default async function DraftSpectatorPage({
  ...
  const [ctx, pools, draft, settings] = await Promise.all([
      ...
      <div className="px-5 py-10 sm:py-14">
        <section className="mx-auto max-w-3xl rounded-[2rem] border border-amber-200/18 bg-amber-200/8 p-6 text-amber-50">
          ...
          <p className="mt-3 text-sm leading-7 text-amber-50/72">
            Das Orga-Team hat Champ Select gerade pausiert. Die Ansicht ist
            wieder erreichbar, sobald der Draft-Schalter aktiv ist.
          </p>
        </section>
      </div>
    );
