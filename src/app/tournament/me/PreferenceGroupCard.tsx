...
export function PreferenceGroupCard({
  ...
  const [pending, setPending] = useState<"create" | "join" | "leave" | null>(
    null,
  );
  ...
  return (
    <section className="rounded-[2rem] border border-cyan-200/14 bg-cyan-300/[0.045] p-5 shadow-xl shadow-black/20 sm:p-6">
      ...
      <p className="mt-3 max-w-2xl text-sm leading-6 text-emerald-100/60">
        Teile deinen privaten Code mit bis zu vier anderen Bewerbern. Die Orga
        sieht euren Wunsch beim Team-Building, eine gemeinsame Einteilung kann
        wegen der Balance aber nicht garantiert werden.
      </p>

      {!hasApplication ? (
        <div className="mt-5 rounded-2xl border border-amber-200/18 bg-amber-200/[0.07] p-4 text-sm text-amber-50/76">
          Speichere zuerst deine Turnierbewerbung, um eine Wunschgruppe zu
          erstellen oder ihr beizutreten.
        </div>
      ) : group ? (
        <div className="mt-5 grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto]">
          ...
          >
            <span>
              <span className="block text-[9px] font-black uppercase tracking-[0.22em] text-cyan-100/42">
                Dein privater Code
              </span>
              <span className="mt-1 block font-mono text-2xl font-black tracking-[0.16em] text-cyan-50">
                {group.code}
              </span>
            </span>
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          <div className="rounded-2xl border border-white/9 bg-black/18 p-4">
            ...
            <p className="mt-1 text-xs leading-5 text-emerald-100/48">
              Erzeuge einen Code und teile ihn privat mit deinen Mitspielern.
            </p>
            <button
              type="button"
          ...
          >
            <label
              htmlFor="preference-group-code"
              className="text-sm font-black text-emerald-50"
            >
              Bestehendem Code beitreten
            </label>
            <p className="mt-1 text-xs leading-5 text-emerald-100/48">
              Den Code erhältst du direkt von einer Person aus der Wunschgruppe.
            </p>
      ...
      {message ? (
        <div
          className={`mt-4 rounded-xl border px-4 py-3 text-xs font-bold ${
            message.tone === "ok"
              ? "border-lime-200/20 bg-lime-200/[0.07] text-lime-50/80"
              : "border-red-200/20 bg-red-500/[0.07] text-red-100"
          }`}
        >
          {message.text}
        </div>
