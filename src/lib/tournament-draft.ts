...
function sanitizeAction(value: unknown): DraftAction | null {
  ...
  if (
    (raw.side !== "teamA" && raw.side !== "teamB")
    || (raw.kind !== "pick" && raw.kind !== "ban")
    || !raw.champion
    || !raw.lockedAt
  ) {
    return null;
...
function strip(doc: DraftDoc): TournamentDraftState {
  const actions = Array.isArray(doc.actions)
    ? doc.actions.map(sanitizeAction).filter((action): action is DraftAction => !!action)
    : legacyActions(doc);
...
export async function markDraftReady(input: {
  matchId: string;
  side: DraftSide;
  readyBy?: string;
  ...
  const next: TournamentDraftState = {
    ...current,
    readyBy,
    currentTurnStartedAt: readyBy.teamA && readyBy.teamB && !current.currentTurnStartedAt
      ? now
      : current.currentTurnStartedAt,
  ...
  const db = await getDb();
  await db.collection<DraftDoc>(COLLECTION).replaceOne(
    { _id: input.matchId },
    next,
    { upsert: true },
  );
...
export async function forceDraftReady(input: {
  matchId: string;
  readyBy?: string;
}): Promise<TournamentDraftState> {
  const current = await getDraftState(input.matchId);
...
export async function undoLastDraftAction(input: {
  matchId: string;
  updatedBy?: string;
}): Promise<TournamentDraftState> {
  const current = await getDraftState(input.matchId);
...
export async function resetDraftState(input: {
  matchId: string;
  resetBy?: string;
  reason: string;
}): Promise<TournamentDraftState> {
...
export async function handleDraftTimeout(input: {
  ...
  if (
    pending
    && pending.side === turn.side
    && pending.kind === turn.kind
    && !alreadyUsed.has(pending.champion)
  ) {
    const now = new Date().toISOString();
    const next: TournamentDraftState = {
      ...current,
