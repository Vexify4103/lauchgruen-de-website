...
async function getDataDragonVersion(): Promise<string> {
  try {
    ...
    return Array.isArray(versions) && typeof versions[0] === "string"
      ? versions[0]
      : FALLBACK_VERSION;
  } catch {
...
export async function getChampionPools(): Promise<ChampionPool[]> {
  ...
  const response = await fetch(
    `${DATA_DRAGON_BASE}/cdn/${version}/data/de_DE/champion.json`,
    { next: { revalidate: 60 * 60 * 24 } },
  );
  ...
  const grouped = new Map<string, ChampionPoolEntry[]>(
    azLetterPools.map((pool) => [pool, []]),
  );
  ...
  return azLetterPools.map((pool) => ({
    pool,
    label: compactPoolLabel(pool),
    champions: [...(grouped.get(pool) ?? [])].sort((a, b) =>
