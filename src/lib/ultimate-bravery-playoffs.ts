...
const GRAND_FINALS: PlayoffDefinition[] = [
...
export function resolveUltimateBraveryPlayoffMatches(input: {
	...
	if (input.format === "undecided" || input.teams.length !== 8) return [];
	const seeds = computeUltimateBraverySwissSeeds(input.swiss, input.teams, input.requiredRounds);
	const definitions =
		input.format === "single-elimination"
			? SINGLE_ELIMINATION
			: input.format === "double-elimination-light"
				? DOUBLE_ELIMINATION_LIGHT
				: [...QUARTERFINALS, ...UPPER_ROUNDS, ...LOWER_ROUNDS, ...GRAND_FINALS];
	...
	function slotLabel(slot: PlayoffSlot) {
		if (slot.kind === "seed") return `Swiss Seed #${slot.seed}`;
