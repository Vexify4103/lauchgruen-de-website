...
export async function getTournamentLiveStreams(
  teams: TournamentTeam[],
  liveTeamNames: string[],
  options?: { previewOffline?: boolean },
): Promise<TournamentLiveStream[]> {
  const relevantTeams = teams.filter((team) => liveTeamNames.includes(team.name));
  const players = relevantTeams.flatMap((team) =>
    team.players
  ...
  const links = (await listTwitchLinksForDiscordIds(discordIds))
    .filter((link) => link.showWhenLive);
