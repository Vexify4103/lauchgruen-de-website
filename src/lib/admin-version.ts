...
export async function getAdminVersion(resource: string): Promise<number> {
  ...
  const doc = await db
    ...
    .findOne({ _id: resource });
  return doc?.version ?? 0;
}

export async function getAdminVersions(
  resources: string[],
  ...
  return Object.fromEntries(
    unique.map((resource) => [resource, byResource.get(resource) ?? 0]),
  );
}
...
export async function claimAdminVersion(input: {
  ...
  | { ok: false; conflict: AdminVersionConflict }
> {
  const db = await getDb();
  ...
  return {
    ...
    conflict: {
      code: "ADMIN_VERSION_CONFLICT",
      message:
        "Diese Daten wurden inzwischen von einer anderen Admin-Person geändert.",
